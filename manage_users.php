...
<aside id="sidebar" class="w-64 bg-white border-r border-gray-200 text-gray-700 flex flex-col transition-all duration-300 ease-in-out">
  ...
  <nav class="flex-1 px-4 py-6 space-y-2">
    ...
    <a href="manage_users.php" class="block px-4 py-2 rounded-lg bg-gray-100 text-blue-600 font-semibold">👥 Manage Users</a>
...
<!-- Main Content -->
<main class="flex-1 p-8 transition-all duration-300 ease-in-out bg-gray-50">
  ...
  <header class="flex justify-between items-center mb-6">
    ...
    <!-- Back button -->
    <a href="superadmin_dashboard.php" class="bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg font-medium shadow hover:bg-gray-100 transition">
      ⬅ Back
    </a>
  </header>
    ...
    <table class="min-w-full border border-gray-200 rounded-lg overflow-hidden">
      ...
      <tbody class="divide-y divide-gray-200">
        <?php while ($row = $result->fetch_assoc()) { ?>
          <tr class="hover:bg-gray-50 transition <?= ($row['status_id'] == 2) ? 'opacity-50 bg-gray-100' : ''; ?>">
            ...
            <td class="px-4 py-2 text-center space-x-3">
              <a href="edit_user.php?id=<?= $row['admin_id']; ?>" class="text-blue-600 hover:underline font-medium">Edit</a>
              <?php if ($row['status_id'] == 1) { ?>
                <a href="toggle_user.php?id=<?= $row['admin_id']; ?>&status=2" class="text-yellow-600 hover:underline font-medium">Deactivate</a>
              <?php } else { ?>
                <a href="toggle_user.php?id=<?= $row['admin_id']; ?>&status=1" class="text-green-600 hover:underline font-medium">Activate</a>
              <?php } ?>
              <a href="delete_user.php?id=<?= $row['admin_id']; ?>" onclick="return confirm('Are you sure you want to delete this user?')" class="text-red-600 hover:underline font-medium">Delete</a>
            </td>
          </tr>
        <?php } ?>
      </tbody>
