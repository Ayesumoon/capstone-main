...
$next_refund_id = null;
$res = $conn->query("SHOW TABLE STATUS LIKE 'refunds'");
if ($res && $row = $res->fetch_assoc()) {
    $next_refund_id = $row['Auto_increment'];
}
...
<div id="refundModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center hidden z-50">
  <div class="bg-white w-full max-w-md rounded-lg shadow-lg p-6 relative">
    <button onclick="closeRefundModal()" class="absolute top-2 right-2 text-gray-500 hover:text-gray-700 text-xl font-bold">&times;</button>
    ...
    <form id="refundForm" method="POST" action="process_refund.php" onsubmit="return confirm('Confirm refund?')">
      ...
      <div class="flex space-x-3 mt-6">
        <button type="submit" class="bg-pink-500 hover:bg-pink-600 text-white px-4 py-2 rounded-lg w-1/2 shadow-md">Submit Refund</button>
        <button type="button" onclick="closeRefundModal()" class="bg-gray-700 hover:bg-gray-800 text-white px-4 py-2 rounded-lg w-1/2 shadow-md">Cancel</button>
      </div>
    </form>
  </div>
</div>
...
<script>
function openRefundModal() { document.getElementById('refundModal').classList.remove('hidden'); }
function closeRefundModal() { document.getElementById('refundModal').classList.add('hidden'); }

function loadOrderItems() {
  const orderId = document.getElementById("refundOrderId").value;
  if (!orderId) { alert("Enter Order ID first."); return; }

  fetch("get_order_items.php?order_id=" + orderId)
    .then(res => res.json())
    .then(items => {
      if (items.length === 0) { alert("No items found for this order."); return; }
      ...
      select.addEventListener("change", () => { document.getElementById("refundQty").max = select.options[select.selectedIndex].dataset.maxQty; });
      select.dispatchEvent(new Event("change"));
    })
    .catch(err => { console.error(err); alert("Error loading order items."); });
}
</script>
  ...
  <div class="flex-1 overflow-y-auto p-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6" id="productGrid">
      <?php foreach($productsArr as $product): ?>
        <?php
          $imagePath = 'uploads/products/' . basename($product['image']);
          if (!file_exists($imagePath) || empty($product['image'])) $imagePath = 'placeholder.png';
        ?>
        <div class="border rounded shadow hover:shadow-lg transform hover:scale-105 transition p-3 text-center product-card"
          ...
          <img src="<?= htmlspecialchars($imagePath) ?>" alt="<?= htmlspecialchars($product['name']) ?>" class="w-full h-32 object-cover rounded mb-2">
          ...
          <button onclick="addVariantToCart(<?= $product['id'] ?>, '<?= addslashes($product['name']) ?>', <?= $product['price'] ?>, this)" 
...
<div class="w-96 bg-pink-100 shadow-lg rounded-lg overflow-hidden">
  ...
  <div class="p-4 flex flex-col flex-1 text-center">
    ...
    <div class="mt-4">
      <button type="button" onclick="openRefundModal()" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg w-full shadow-md">
        Refund
      </button>
    </div>
  </div>
...
document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll(".product-card").forEach(card => {
    ...
    colorSelect.addEventListener("change", () => {
      ...
      if (chosenSize) sizeSelect.value = chosenSize;
    });
  });
});
...
function addVariantToCart(pid, name, price, btn) {
  ...
  if (!size || !colorSelect.value) { alert("Please select both size and color."); return; }

  const stock_id = parseInt(selectedColor.dataset.stockId);
  if (!stock_id) { alert("Invalid size/color combination."); return; }
...
function addToCart(stock_id, name, price, color, size) {
  ...
  if (item) item.qty++; else cart.push({stock_id, name, price, color, size, qty: 1});
  renderCart();
}
...
function renderCart() {
  ...
  if (cart.length === 0) { cartDiv.classList.add("hidden"); emptyMsg.classList.remove("hidden"); }
  else {
    cartDiv.classList.remove("hidden"); emptyMsg.classList.add("hidden");
    cart.forEach((item,i) => {
...
function filterCategory(category, e) {
  ...
  products.forEach(p => { p.style.display = (category==="all"||p.dataset.category.toLowerCase()===category) ? "block" : "none"; });
